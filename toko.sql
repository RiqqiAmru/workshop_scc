...
(2, 'aaa', 1, 50000, 2, 10000);

--
-- Triggers `penjualan_detail`
--
DELIMITER $$
CREATE TRIGGER `penjualan_detail_ad` AFTER DELETE ON `penjualan_detail` FOR EACH ROW begin
update barang set stok=stok+old.jumlah where
barang_id=old.barang_id;
update penjualan set total=total-old.subtotal where
nomor_nota=old.nomor_nota;
end
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `penjualan_detail_ai` AFTER INSERT ON `penjualan_detail` FOR EACH ROW begin
update barang set stok=stok-new.jumlah where
barang_id=new.barang_id;
update penjualan set total=total+new.subtotal where
...
(1, 'admin 1', 'admin1', 'admin1', 'admin1');
...
--
ALTER TABLE `penjualan_detail`
  MODIFY `penjualan_detail_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
...
--
ALTER TABLE `penjualan_detail`
  ADD CONSTRAINT `penjualan_detail_ibfk_3` FOREIGN KEY (`nomor_nota`) REFERENCES `penjualan` (`nomor_nota`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `penjualan_detail_ibfk_4` FOREIGN KEY (`barang_id`) REFERENCES `barang` (`barang_id`) ON DELETE CASCADE ON UPDATE CASCADE;
COMMIT;
